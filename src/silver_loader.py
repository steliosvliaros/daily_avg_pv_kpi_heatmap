...
def load_silver_wide(
    ...
    park_ids : list-like, optional
        Filter to parks containing any of the provided words (case-insensitive).
        Matches if park_id contains any word from the list.
    signals : list-like, optional
        Filter to signals containing any of the provided words (case-insensitive).
        Matches if signal_name contains any word from the list.
    ...
    if park_set is not None:
        # Match if park_id contains any word from the list
        park_ids_lower = df["park_id"].astype(str).str.lower()
        mask = park_ids_lower.str.contains('|'.join(park_set), case=False, na=False, regex=True)
        df = df[mask]
        if debug:
            print(f"[load_silver_wide] rows after park filter (contains any of {list(park_set)}): {len(df)}")

    if signal_set is not None:
        # Match if signal_name contains any word from the list
        signal_names_lower = df["signal_name"].astype(str).str.lower()
        mask = signal_names_lower.str.contains('|'.join(signal_set), case=False, na=False, regex=True)
        df = df[mask]
        if debug:
            print(f"[load_silver_wide] rows after signal filter (contains any of {list(signal_set)}): {len(df)}")
...
def load_silver(
    ...
    # Apply park filter - match if park_id contains any word from the list
    if park_set is not None:
        park_ids_lower = df["park_id"].astype(str).str.lower()
        mask = park_ids_lower.str.contains('|'.join(park_set), case=False, na=False, regex=True)
        df = df[mask]
        if debug:
            print(f"[load_silver] rows after park filter (contains any of {list(park_set)}): {len(df)}")
    
    # Apply signal filter - match if signal_name contains any word from the list
    if signal_set is not None:
        signal_names_lower = df["signal_name"].astype(str).str.lower()
        # Create a mask that is True if ANY word from signal_set is in the signal_name
        mask = signal_names_lower.str.contains('|'.join(signal_set), case=False, na=False, regex=True)
        df = df[mask]
        if debug:
            print(f"[load_silver] rows after signal filter (contains any of {list(signal_set)}): {len(df)}")
...
def load_daily_energy_wide(
    ...
    only_valid=True,
    interval_minutes=15,
    keep_signal_detail=False,
    flatten_columns=False,
    persist_chunk_dir=None,
    persist_final_path=None,
    cleanup_chunks=False,
    log_every=1,
    debug=False,
):
    """
    Load power data and compute daily total energy (kWh) per park in wide format.

    Assumes power measurements are average values over fixed intervals (default 15 minutes).
    Energy per interval is computed as: power_kW * (interval_minutes / 60).
    ...
    interval_minutes : int, default 15
        Interval length in minutes for power readings
    keep_signal_detail : bool, default False
        If True, keep separate columns per (park_id, signal_name, unit)
        If False, sum all signals per park_id
    flatten_columns : bool, default False
        If True and keep_signal_detail=True, flatten MultiIndex columns to strings
    persist_chunk_dir : Path or str, optional
        If provided, write per-partition daily energy parquet files to this directory
    persist_final_path : Path or str, optional
        If provided, write the final daily energy DataFrame to this parquet path
    cleanup_chunks : bool, default False
        If True and persist_chunk_dir is set, delete chunk files after final write
    log_every : int, default 1
        Log progress every N partition files when debug=True
    debug : bool, default False
        If True, print diagnostic information
    ...
    -------
    pd.DataFrame
        Daily total energy (kWh) wide DataFrame indexed by date.
        Columns are park_id (default) or full signal detail if keep_signal_detail=True.
    """
    ...
    part_files = _iter_silver_parts(silver_root, start_date=start_date, end_date=end_date)
    if debug:
        print(f"[load_daily_energy_wide] part files: {len(part_files)}")
    if not part_files:
        return pd.DataFrame()
    ...
    daily_energy = None
    chunk_dir = Path(persist_chunk_dir) if persist_chunk_dir else None
    if chunk_dir:
        chunk_dir.mkdir(parents=True, exist_ok=True)

    for idx, pf in enumerate(part_files, start=1):
        if debug and (idx % log_every == 0 or idx == 1 or idx == len(part_files)):
            print(f"[load_daily_energy_wide] Processing {idx}/{len(part_files)}: {pf.name}")
        df = _read_parquet_columns(pf, use_cols)
        if df.empty:
            continue
        ...
        if only_valid:
            if "value" in df.columns:
                df = df[df["value"].notna()]
            flag_cols_present = [c for c in df.columns if c.startswith("flag_")]
            if flag_cols_present:
                df = df[~df[flag_cols_present].fillna(False).any(axis=1)]
            if df.empty:
                continue

        df = df.copy()
        df["_day"] = df[time_col].dt.normalize()

        # Unit-aware conversion to kWh per interval
        unit_series = df["unit"].astype(str).str.strip().str.lower() if "unit" in df.columns else None
        value = pd.to_numeric(df["value"], errors="coerce")
        if unit_series is None:
            energy_value = value * interval_hours
        else:
            ...
            # Power units: convert to kW then multiply by interval_hours
            energy_value.loc[is_w] = energy_value.loc[is_w] / 1000.0
            energy_value.loc[is_kw] = energy_value.loc[is_kw] * 1.0
            energy_value.loc[is_w | is_kw] = energy_value.loc[is_w | is_kw] * interval_hours

            # Energy units: convert to kWh, no interval scaling
            energy_value.loc[is_wh] = energy_value.loc[is_wh] / 1000.0
            energy_value.loc[is_kwh] = energy_value.loc[is_kwh] * 1.0
            # Fallback: treat unknown units as power in kW
            unknown_mask = ~(is_w | is_kw | is_wh | is_kwh)
            energy_value.loc[unknown_mask] = energy_value.loc[unknown_mask] * interval_hours

        if keep_signal_detail:
            group_cols = ["_day", "park_id", "signal_name"]
            if "unit" in df.columns:
                group_cols.append("unit")
            grouped = df.assign(_energy=energy_value).groupby(group_cols, dropna=False)["_energy"].sum()
            if "unit" in df.columns:
                chunk_wide = grouped.unstack(["park_id", "signal_name", "unit"])
            else:
                chunk_wide = grouped.unstack(["park_id", "signal_name"])
        else:
            grouped = df.assign(_energy=energy_value).groupby(["_day", "park_id"], dropna=False)["_energy"].sum()
            chunk_wide = grouped.unstack("park_id")

        if chunk_dir:
            chunk_path = chunk_dir / f"daily_energy_{pf.stem}.parquet"
            chunk_wide.to_parquet(chunk_path)

        if daily_energy is None:
            daily_energy = chunk_wide
        else:
            daily_energy = daily_energy.add(chunk_wide, fill_value=0)

    if daily_energy is None or daily_energy.empty:
        if debug:
            print("[load_daily_energy_wide] No data loaded, returning empty DataFrame")
        return pd.DataFrame()

    daily_energy = daily_energy.sort_index()

    if keep_signal_detail and flatten_columns and isinstance(daily_energy.columns, pd.MultiIndex):
        daily_energy.columns = ["__".join(str(c) for c in col).strip("__") for col in daily_energy.columns]

    # Legacy persist_final_path support (direct write without gold ingestion)
    if persist_final_path:
        final_path = Path(persist_final_path)
        final_path.parent.mkdir(parents=True, exist_ok=True)
        daily_energy.to_parquet(final_path)

    if cleanup_chunks and chunk_dir:
        shutil.rmtree(chunk_dir, ignore_errors=True)
        if debug:
            print(f"[load_daily_energy_wide] Removed chunk directory: {chunk_dir}")

    if debug:
        print(f"[load_daily_energy_wide] Output shape: {daily_energy.shape}")

    return daily_energy


def load_and_ingest_daily_energy_gold(
    silver_root,
    metadata_path,
    gold_root,
    park_ids=None,
    ...
    """
    Load daily energy from silver and safely ingest to gold layer.
    
    This is the recommended function for gold layer ingestion. It combines
    load_daily_energy_wide() with safe ingestion logic that prevents duplicates
    and respects datetime boundaries using watermark tracking.
    ...
    metadata_path : Path or str
        Path to park_metadata.csv file
    gold_root : Path or str
        Root directory of gold layer
    park_ids : list-like, optional
        Filter to parks containing any of the provided words
    signals : list-like, optional
        Filter to signals containing any of the provided words
    start_date, end_date : str or Timestamp, optional
        Date range for loading (inclusive)
    status_effective : str or list-like, default "true"
    ...
    force_full_replace : bool, default False
        If True, replace entire gold dataset (ignore watermark)
    debug : bool, default False
        If True, print diagnostic information
    ...
    -------
    dict
        Ingestion result from gold_ingest.ingest_daily_energy_safe()
        Contains keys: status, rows_written, last_date, message
    ...
    ... )
    >>> print(result)
    {'status': 'appended', 'rows_written': 150, 'last_date': ..., 'message': '...'}
    """
    from src.gold_ingest import GoldIngestionConfig, ingest_daily_energy_safe
    
    # Load daily energy from silver
    if debug:
        print("[load_and_ingest_daily_energy_gold] Loading daily energy from silver...")
    
    daily_energy_df = load_daily_energy_wide(
        silver_root=silver_root,
        metadata_path=metadata_path,
        park_ids=park_ids,
        signals=signals,
        ...
        keep_signal_detail=False,  # Always aggregate by park_id for gold
        flatten_columns=False,
        persist_chunk_dir=None,  # Don't persist chunks when using gold ingestion
        persist_final_path=None,  # Don't use legacy persist
        cleanup_chunks=False,
        log_every=10,
        debug=debug,
    )
    ...
    if daily_energy_df.empty:
        if debug:
            print("[load_and_ingest_daily_energy_gold] No data loaded from silver")
        return {
            "status": "skipped",
            "rows_written": 0,
            "last_date": None,
            "message": "No data loaded from silver"
        }
    
    # Ingest to gold
    gold_config = GoldIngestionConfig(gold_root=gold_root, table_name="daily_energy")
    
    if debug:
        print(f"[load_and_ingest_daily_energy_gold] Ingesting to gold: {gold_config.table_dir}")
    
    result = ingest_daily_energy_safe(
        daily_energy_df=daily_energy_df,
        config=gold_config,
    ...
    if debug:
        print(f"[load_and_ingest_daily_energy_gold] Result: {result}")
    
    return result


def load_gold_daily_energy(gold_root, debug=False) -> pd.DataFrame:
    """
    Load daily energy from gold layer.
    ...
    -------
    pd.DataFrame
        Daily energy DataFrame indexed by date, or empty DataFrame if not found
    """
    from src.gold_ingest import GoldIngestionConfig, load_gold_table
    
    config = GoldIngestionConfig(gold_root=gold_root, table_name="daily_energy")
    return load_gold_table(config=config, debug=debug)


def load_and_ingest_pvgis_reference_gold(
    pvgis_path,
    metadata_path,
    gold_root,
    park_ids=None,
    ...
    """
    Load PVGIS reference data and safely ingest to gold layer.
    
    Uses hash-based change detection to avoid unnecessary rewrites.
    
    Parameters
    ----------
    pvgis_path : Path or str
        Path to PVGIS parquet file or directory
    metadata_path : Path or str
        Path to park_metadata.csv file
    gold_root : Path or str
        Root directory of gold layer
    ...
    -------
    dict
        Ingestion result from gold_ingest.ingest_pvgis_reference_safe()
        Contains keys: status, rows_written, data_hash, message
    """
    from src.gold_ingest import GoldIngestionConfig, ingest_pvgis_reference_safe
    
    # Load PVGIS data
    if debug:
        print("[load_and_ingest_pvgis_reference_gold] Loading PVGIS reference data...")
    
    pvgis_df = load_pvgis_filtered_wide(
        pvgis_path=pvgis_path,
        metadata_path=metadata_path,
        park_ids=park_ids,
        signals=signals,
    ...
    if pvgis_df.empty:
        if debug:
            print("[load_and_ingest_pvgis_reference_gold] No PVGIS data loaded")
        return {
            "status": "skipped",
            "rows_written": 0,
            "data_hash": "",
            "message": "No PVGIS data loaded"
        }
    
    # Ingest to gold
    gold_config = GoldIngestionConfig(gold_root=gold_root, table_name="pvgis_reference")
    
    if debug:
        print(f"[load_and_ingest_pvgis_reference_gold] Ingesting to gold: {gold_config.table_dir}")
    
    result = ingest_pvgis_reference_safe(
        pvgis_df=pvgis_df,
        config=gold_config,
    ...
    if debug:
        print(f"[load_and_ingest_pvgis_reference_gold] Result: {result}")
    
    return result


def load_gold_pvgis_reference(gold_root, debug=False) -> pd.DataFrame:
    """
    Load PVGIS reference data from gold layer.
    ...
    -------
    pd.DataFrame
        PVGIS reference DataFrame indexed by date, or empty DataFrame if not found
    """
    from src.gold_ingest import GoldIngestionConfig, load_gold_table
    
    config = GoldIngestionConfig(gold_root=gold_root, table_name="pvgis_reference")
    return load_gold_table(config=config, debug=debug)
