...
class BronzePipelineConfig:
    ...
    # Parquet input options (alternative to folder ingestion)
    parquet_input_path: Optional[Path] = None  # If set, ingest this parquet file directly
    parquet_chunk_size: int = 250_000  # Rows per chunk
    ...
    try:
        ...
        if config.park_metadata_path and config.park_metadata_path.exists():
            try:
                ...
                park_ids_raw = meta.loc[status_series == "true", "park_id"]
                park_ids_clean = (
                    park_ids_raw
                    .astype("string")
                    .str.strip()
                ...
                invalid_mask = park_ids_clean.isna() | (park_ids_clean == "")
                dropped_count = int(invalid_mask.sum())
                allowed_parks = set(park_ids_clean.loc[~invalid_mask].tolist())
                print(f"[bronze_pipeline] status_effective=true parks: {len(allowed_parks)}")
                if dropped_count > 0:
                    print(f"[bronze_pipeline] Dropped {dropped_count} invalid park_id values from metadata")
                print(f"[bronze_pipeline] Sample allowed park_ids: {sorted(allowed_parks)[:5]}")
        ...
        if config.run_ingestion:
            ...
            # Check if parquet input is specified
            if config.parquet_input_path:
                print(f"Ingesting parquet file directly: {config.parquet_input_path.name}...")
                # Copy parquet to inbox so ingest_folder can process it
                import shutil
                parquet_in_inbox = ingest_cfg.inbox / config.parquet_input_path.name
                if not parquet_in_inbox.exists():
                    shutil.copy2(config.parquet_input_path, parquet_in_inbox)
                    print(f"  Copied to inbox: {parquet_in_inbox.name}")
                
                # Update config for chunking
                ingest_cfg.parquet_chunk_size = config.parquet_chunk_size
            else:
