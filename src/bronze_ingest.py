...
- Watches a landing folder (data_root/inbox) for Excel/CSV/Parquet exports
...
- Else:
    - Reads the file (supports .xlsx, .csv, .parquet)
    ...
    - For parquet: reads in configurable chunks (default: 250k rows) for memory efficiency
...
class Config:
    ...
    parquet_chunk_size: int = 250_000  # Rows per chunk for parquet files
...
def registry_has_hash(
    registry_df: pd.DataFrame,
    dataset: str,
    file_hash: str,
    eligible_statuses: Tuple[str, ...] = ("ingested",),
) -> bool:
    ...
    sub = registry_df[
        ...
        & (registry_df["source_file_hash"] == file_hash)
    ]
    if sub.empty:
        return False
    ...
    status_series = sub["status"].astype(str).str.strip().str.lower()
    allowed = {s.strip().lower() for s in eligible_statuses}
    return status_series.isin(allowed).any()
...
def read_parquet_chunked(path: Path, chunk_size: int = 250_000) -> List[pd.DataFrame]:
    """
    Read a parquet file in chunks.
    
    Args:
        path: Path to parquet file
        chunk_size: Number of rows per chunk (default: 250,000)
        
    Returns:
        List of DataFrames, each with at most chunk_size rows
    """
    ...
    print(f"  Reading {total_rows:,} rows from parquet in chunks of {chunk_size:,}")
    
    for i in range(pq_file.num_row_groups):
        table = pq_file.read_row_group(i)
        df_chunk = table.to_pandas()
        
        # If chunk is larger than chunk_size, split it further
        if len(df_chunk) > chunk_size:
            n_splits = int(np.ceil(len(df_chunk) / chunk_size))
            chunk_list = np.array_split(df_chunk, n_splits)
...
def ingest_one_parquet_file(
    ...
    """
    Ingest a parquet file in chunks.
    ...
    Args:
        inbox_file: Parquet file to ingest
        mapping: Column mapping dict
        cfg: Config object
        registry_df: Current registry dataframe
        mapping_filename: Name of mapping file
        mapping_file_hash: SHA256 hash of mapping file
        allow_duplicates: Allow re-ingestion of same file
    ...
    try:
        ...
        print(f"  Ingesting parquet file: {proc_path.name}")
        chunks = read_parquet_chunked(proc_path, chunk_size=chunk_size)

        # Process each chunk
        for chunk_idx, df_wide in enumerate(chunks, 1):
            print(f"  Processing chunk {chunk_idx}/{len(chunks)} ({len(df_wide)} rows)...")
            ...
            # Write Bronze
            files_chunk = write_bronze_monthly(long_df, cfg, run_id=run, file_hash=file_hash, chunk_index=chunk_idx)
            written_files.extend(files_chunk)
            total_rows_long += len(long_df)

        # Archive source file (success)
        archived_path = move_to_archive(proc_path, cfg, file_hash, bucket="archived")

        status_note = "duplicate hash re-ingested (allow_duplicates)" if duplicate_seen else "parquet chunked ingestion ok"
        ...
        write_runlog(cfg, run, {
            ...
            "mapping_filename": mapping_filename,
            "mapping_file_hash": mapping_file_hash,
            "rows_long": int(total_rows_long),
            "files_written": len(written_files),
            "message": status_note,
        })

        return "ingested", f"parquet ingested: {total_rows_long:,} rows, {len(written_files)} files written"
...
def wide_to_long(df_wide: pd.DataFrame, cfg: Config, source_file: str, source_file_hash: str, run_id: str, park_metadata: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    if park_metadata is not None and not park_metadata.empty:
        ...
        park_id_series = park_metadata["park_id"].astype("string").str.strip().str.lower()
        park_id_series = park_id_series[park_id_series.notna() & (park_id_series != "")]
        park_id_set = set(park_id_series.tolist())

        print(f"[bronze_ingest] DEBUG: Metadata has {len(park_id_set)} unique park_ids")
        print(f"[bronze_ingest] DEBUG: Sample metadata park_ids: {sorted(park_id_set)[:5]}")
        ...
        if long_df["park_id"].isna().any():
            ...
            unmatched_clean = [p for p in unmatched_parks if pd.notna(p) and str(p).strip()]
            print(f"[bronze_ingest] WARNING: {nan_count:,} rows have NaN park_id (parks not found in metadata)")
            print(f"[bronze_ingest] Unmatched extracted park_ids: {sorted(unmatched_clean)}")
    else:
...
def write_bronze_monthly(long_df: pd.DataFrame, cfg: Config, run_id: str, file_hash: str, chunk_index: int = 0) -> List[str]:
    ...
    for (y, m), g in long_df.groupby(["year", "month"], dropna=False):
        ...
        # If chunk_index > 0, add it to filename to avoid collisions when writing chunks
        if chunk_index > 0:
            out_file = out_dir / f"part-run={run_id}-hash={file_hash[:12]}-chunk={chunk_index:03d}.parquet"
        else:
...
def list_inbox_files(cfg: Config) -> List[Path]:
    allowed = {".xlsx", ".xls", ".xlsm", ".csv", ".txt", ".parquet"}
    files = []
...
def ingest_folder(cfg: Config, mapping_filename: Optional[str] = None) -> None:
    ...
    try:
        ...
        for f in files:
            # Route to appropriate ingestion function based on file type
            if f.suffix.lower() == ".parquet":
                status, msg = ingest_one_parquet_file(
                    f,
                    mapping,
                    cfg,
                    registry_df,
                    ...
                    chunk_size=cfg.parquet_chunk_size,
                )
            else:
