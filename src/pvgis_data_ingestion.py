...
def build_pvgis_expected_daily_wide_from_metadata(
    metadata_path: Path,
    *,
    cache_root: Optional[Path] = None,
    start_year: int = 2015,
    ...
    """
    Build a wide daily expected-kWh dataframe from park metadata using PVGIS.

    Filters parks to:
    - ``status_effective == true``
    - ``park_iso_name`` present (not NaN/empty)
    - valid ``longitude``, ``latitude``, ``capacity_kwp`` (>0)

    Returns
    -------
    pd.DataFrame
        Wide dataframe with DatetimeIndex (daily dates in ``reference_year``)
        and one column per ``park_iso_name`` containing expected daily kWh.
    """
    ...
    required_cols = {"status_effective", "park_iso_name", "latitude", "longitude", "capacity_kwp"}
    missing_cols = required_cols.difference(metadata.columns)
    if missing_cols:
        raise ValueError(f"park_metadata missing required columns: {sorted(missing_cols)}")

    df = metadata.copy()
    status_series = df["status_effective"].astype("string").str.strip().str.lower()
    iso_series = df["park_iso_name"].astype("string").str.strip()
    df = df[(status_series == "true") & iso_series.notna() & iso_series.ne("")].copy()

    if df.empty:
        raise ValueError("No parks matched filters: status_effective=true and park_iso_name not null")

    for col in ["latitude", "longitude", "capacity_kwp"]:
        _coerce_numeric(df, col)

    df = df[df["capacity_kwp"] > 0].copy()
    df = df.dropna(subset=["latitude", "longitude", "capacity_kwp"])

    if df.empty:
        raise ValueError("No parks with valid latitude/longitude/capacity_kwp after filtering")
    ...
    if "park_name" not in df.columns:
        df["park_name"] = df["park_iso_name"]

    df["park_iso_name"] = df["park_iso_name"].astype("string").str.strip()
    duplicate_iso = df["park_iso_name"].duplicated(keep="last")
    if duplicate_iso.any():
        dup_count = int(duplicate_iso.sum())
        print(f"Warning: Dropping {dup_count} duplicate park_iso_name rows (keeping last)")
        df = df[~duplicate_iso].copy()

    start_year, end_year = _clamp_years(start_year, end_year)
    resolved_cache_root = Path(cache_root) if cache_root is not None else (Path.cwd() / "pvgis" / "pvgis_cache")
    daily_cache_dir = resolved_cache_root / "typical_daily"

    if save_cache:
        ensure_dir(resolved_cache_root)
        ensure_dir(daily_cache_dir)

    series_by_park: Dict[str, pd.Series] = {}

    for row in tqdm(df.itertuples(index=False), total=len(df), desc="PVGIS wide expected daily"):
        park_iso_name = str(getattr(row, "park_iso_name")).strip()
        park_id_value = str(getattr(row, "park_id", "")).strip().lower()
        if not park_id_value or park_id_value == "nan":
            park_id_value = park_iso_name.lower()

        row_series = pd.Series(
            {
                "park_id": park_id_value,
                "park_name": str(getattr(row, "park_name", getattr(row, "park_iso_name"))),
                "latitude": float(getattr(row, "latitude")),
                "longitude": float(getattr(row, "longitude")),
                "capacity_kwp": float(getattr(row, "capacity_kwp")),
                "timezone": str(getattr(row, "timezone")),
            }
        )

        daily_df, _, _ = fetch_typical_daily_for_park(
            row_series,
            start_year=start_year,
            end_year=end_year,
            loss_pct=loss_pct,
            reference_year=reference_year,
            drop_feb29=drop_feb29,
            hourly_cache_dir=resolved_cache_root,
            daily_cache_dir=daily_cache_dir,
            pvgis_url=pvgis_url,
        ...
        park_series = daily_df[["interval_start_date", "value"]].copy()
        park_series["interval_start_date"] = pd.to_datetime(park_series["interval_start_date"], errors="coerce")
        park_series = park_series.dropna(subset=["interval_start_date"]).set_index("interval_start_date")["value"]
        park_series = park_series.sort_index()
        park_series.name = park_iso_name
        series_by_park[park_iso_name] = park_series

    if not series_by_park:
        return pd.DataFrame()

    wide_df = pd.concat(series_by_park, axis=1).sort_index()
    wide_df.index.name = "interval_start_date"
    return wide_df
...
__all__ = [
    "TypicalYearConfig",
    "build_park_meta",
    "build_pvgis_typical_year_dataset",
    "build_pvgis_expected_daily_wide_from_metadata",
]
